...
/**
 * @fileoverview query data from svn with 401 auth.
...
function bind(resp, opts, callback) {
  ...
  resp.on('end', function() {
    if (resp.headers['content-type'] !== 'application/octet-stream')
      data = rmsUtil.gbk2utf8(data).toString();
    ...
    if (opts.path[opts.path.length - 1] !== '/') {
      ...
      try {
        ...
        callback(null, obj.svn.index, obj.svn.index.rev);
      } catch (e) {
        rmsUtil.error('Subversoin Error: %s%s', opts.host + opts.path);
        callback(new Error('Subversion Error: ' + e.message));
      }
    }
  });
}
