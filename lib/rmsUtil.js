...
rmsUtil.__defineGetter__('now', function() {
  return rmsUtil.formatDateTime(new Date);
});

rmsUtil.formatDateTime = function(date) {
  if (typeof date === 'string') date = new Date(date);
  if (_(+date).isNaN()) return '';
  return util.format(
    '%s-%s-%s %s:%s:%s',
    date.getFullYear(),
...
rmsUtil.isTxt = function(exts) {
  var config = require('./rmsConfig');
  return new RegExp('\.(' + config.txtExts.join('|') + ')').test(exts);
};

rmsUtil.isImg = function(exts) {
  var config = require('./rmsConfig');
  return new RegExp('\.(' + config.imgExts.join('|') + ')').test(exts);
};

rmsUtil.dbError = function(err, res) {
