...
asyncWorkflow.prototype.next = function(content) {
  ...
  self.steps[self.current].run(self.content, function(nextContent) {
    if (!nextContent) {
      throw new asyncWorkflow.WORKFLOW_NOT_COMPLETE;
    }
...
asyncWorkflow.__defineGetter__('WORKFLOW_NOT_COMPLETE', function() {
  return WORKFLOW_NOT_COMPLETE;
});
...
function WORKFLOW_NOT_COMPLETE() {
  Error.apply(this, arguments);
  this.message = 'Workflow not complete';
}
util.inherits(WORKFLOW_NOT_COMPLETE, Error);
// }}}
