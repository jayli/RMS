...
module.exports = function(req, res, apptype, appname, filepath, self, o) {
  ...
  if (_.isString(o)) {
    ...
    // TODO for supporting version
    filelist[RmsUtil.md5(self.svnroot)] = self.svnroot;
    ...
    var ids = _(client.config.filelist).keys();
    res.local('ids', ids.join('|'));

    res.local('filelist', _(client.config.filelist).map(function(path, idx) {
      return {id: idx, path: path};
    }));
