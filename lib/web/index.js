...
app.get('/:apptype/:appname?/:action?/*?', function(req, res) {
  ...
  // TODO 路由规则需要仔细设计
  if (req.params[0]) {
    ['appname', 'action'].some(function(k, i) {
      if (slash <= i + 2) {
  ...
  if (apptype in WebConfig.apps) {
    ...
    if (!appname) {
      ...
      if (appname === 'new') {
        ...
        if (action === 'delete') {
          appModel.delete(appname, function(err, results, fileds) {
            if (err) {
              res.local('message', {
                type: 'error',
                text: err.message
    ...
    res.local('error', 'App type not supported. ' + apptype);
    res.render('error');
  }
});
