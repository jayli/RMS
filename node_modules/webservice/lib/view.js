...
//       var path = "/" + method;
      ...
      if(typeof m.regex !== 'undefined') {
       ...
       html += ('<pre class="curl">curl -X GET '  + items.endpoint +  '/' + method + sampleCurlArgs + '</pre>');
       ...
       html += ('<pre class="curl">curl -X POST -d "' + sampleCurlArgs.substr(1, sampleCurlArgs.length) + '" ' + items.endpoint + '/' + method + '</pre>');
       html += ('You can also post JSON...<br/>');
       html += ('<pre class="curl">curl -X POST -d "' + JSON.stringify(sampleCurlJSON) + '" ' + items.endpoint + '/' + method + '</pre>');
